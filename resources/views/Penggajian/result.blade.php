											...
											<tr>
												...
												@foreach($tunjangan as $data1)
													...
													@if($data1->pegawai_id == $data->id)
														{{$data1->Tunjangan->besaran_uang}}
														@php $a=$data1->Tunjangan->besaran_uang; ; @endphp
													@endif
												...
												@foreach($lembur as $data2)
													@if($data2->pegawai_id == $data->id)
														{{$data2->Jumlah_jam}}
														@php $b=$data2->Jumlah_jam*$data2->KategoryPegawai->besaran_uang; @endphp
												...
												@foreach($lembur as $data2)
													@if($data2->pegawai_id == $data->id)
														{{$data2->Jumlah_jam*$data2->KategoryLembur->besaran_uang}}
														@php $b=$data2->Jumlah_jam*$data2->KategoryLembur->besaran_uang; @endphp

													@endif
												@endforeach
												</td>
												<td>{{$data->Golongan->besaran_uang+$data->jabatan->besaran_uang}}</td>
												@php $c=$data->Golongan->besaran_uang+$data->jabatan->besaran_uang; @endphp
							                        ...
							                        {{ csrf_field() }}
							                        	...
							                        	@foreach($lembur as $data2)
															@if($data2->pegawai_id == $data->id)
																...
																<input type="hidden" name="jumlah_uang_lembur" value="{{$data2->jumlah_jam*$data2->kategori->besaran_uang}}">
															@endif
														@endforeach
														<input type="hidden" name="gaji_pokok" value="{{$data->golongan->besaran_uang+$data->jabatan->besaran_uang}}">
